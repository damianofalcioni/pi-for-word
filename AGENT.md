...
- `**src/assistant-tools/**` — Pi Agent tools for ChatPanel: `**createWordTools()**` (Word `**Word.run**`: selection, outline, search, insert Markdown) plus `**createPi4WordJavaScriptReplTool**` (pi-web-ui `**javascript_repl**` wrapper with `**code` / `script**` normalization). `**index.js**` is the barrel; `**assistant**` re-exports `**createWordTools**` from `**pi-assistant.js**`. `**task-pane**` wires tools via `**assistant**` imports and `**mountChatPanel**` (adds the REPL alongside `**createWordTools()**`).
...
**Pi4Word:** `**SYSTEM_PROMPT`** lives in `**src/assistant/pi-assistant.js**`; **Word / chat tools** live under `**src/assistant-tools/`** (`**index.js**` exports `**createWordTools**`; `**word-tool-get-selection.js**`, `**word-tool-get-document-outline.js**`, `**word-tool-search-text.js**`, `**word-tool-insert-markdown.js**`, `**word-tools-search.js**`, `**word-tools-shared.js**`, `**javascript-repl-tool.js**`). When you change selection/read semantics, insert placement, outline behavior, search anchoring, streaming/settings keys, or model instructions, update `**SPECS.md**` (product summary and **Assistant** / **Networking** bullets) so spec, prompt, and tools stay aligned.